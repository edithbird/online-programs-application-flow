---
title: "Flow of Applications: Online Programs"
subtitle: ""
author: "Office ofMarketing and Brand Management"
date: "`r Sys.Date()`"
...
NB: This `tintBook` variant is for book-length document in pdf; there are is also
short `tintPdf` variant suitable for handouts or articles along with a
similar `tintHtml` variant for shorter web-based documents.  There is
currently no book-length html variant as we find all current html / css / js
offerings for this unsatisfactory. Also note that we use the exact same input as 
for the other two formats and have not yet customized this for a book.
